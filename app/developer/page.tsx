...
export default function DeveloperDashboard() {
  ...
  const statusDistribution = [
    ...
    { label: 'TAC', count: (aggregates.byPhase['TAC_SCHEDULED'] || 0) + (aggregates.byPhase['TAC_CONFIRMED'] || 0), color: 'bg-blue-400' },
    { label: 'Dihantar', count: aggregates.byPhase['SUBMITTED'] || 0, color: 'bg-purple-400' },
    { label: 'LO/KJ', count: (aggregates.byPhase['LO_RECEIVED'] || 0) + (aggregates.byPhase['KJ_PENDING'] || 0), color: 'bg-orange-400' },
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="bg-slate-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-slate-800 transition-colors"
            >
      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl p-6 mb-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-slate-400 text-sm mb-1">Projek</p>
              <h2 className="text-2xl font-bold">{projectInfo.name}</h2>
              <p className="text-slate-300">{projectInfo.location}</p>
            </div>
            <div className="text-right">
              <p className="text-slate-400 text-sm">Jumlah Unit</p>
              <p className="text-3xl font-bold">{projectInfo.totalUnits}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-700">
            <div>
              <p className="text-slate-400 text-sm">Dijual</p>
              <p className="text-2xl font-bold text-green-400">{projectInfo.sold}</p>
            </div>
            <div>
              <p className="text-slate-400 text-sm">Pinjaman Dalam Proses</p>
              <p className="text-2xl font-bold text-orange-400">{projectInfo.loanInProgress}</p>
            </div>
            <div>
              <p className="text-slate-400 text-sm">Kadar Penukaran</p>
              <p className="text-2xl font-bold text-blue-400">{aggregates.conversionRate}%</p>
            </div>
          </div>
        </div>
              ...
              {proofEvents.slice(0, 10).map(event => (
                ...
                >
                  <div className={`w-2 h-2 rounded-full ${
                    event.category === 'FACT' ? 'bg-blue-500' :
