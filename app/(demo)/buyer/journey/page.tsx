      ...
      case 0:
        return (
          <div className="text-center py-6">
            ...
            <div className="bg-snang-teal-50 border border-snang-teal-200 rounded-xl p-4 mb-5 text-left">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-snang-teal-800">
                  ...
                  <ul className="space-y-1 text-snang-teal-700">
      ...
      case 1:
        return (
          <div>
            ...
            {/* Progress Bar */}
            <div className="bg-gradient-to-r from-snang-teal-700 to-snang-teal-900 rounded-xl p-4 mb-5">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium text-slate-300">Kesediaan Dokumen</span>
                <span className="text-lg font-bold text-snang-teal-500">{readinessPercent}%</span>
              </div>
              <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-snang-teal-500 to-snang-teal-400 transition-all duration-500"
                ...
                return (
                  <div 
                    ...
                    className={`border-2 rounded-xl p-4 transition-all ${
                      uploaded 
                        ? 'border-snang-teal-200 bg-snang-teal-50' 
                        : 'border-slate-200 bg-white'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        uploaded ? 'bg-snang-teal-600' : 'bg-slate-100'
                      }`}>
                        {uploaded 
                      ...
                      {uploaded ? (
                        <button 
                          ...
                          className="text-snang-teal-600 hover:text-snang-teal-600"
                        >
                        ...
                        <button 
                          ...
                          className="bg-snang-teal-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-snang-teal-700"
                        >
                          Muat Naik
                        </button>
                      )}
                      ...
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {isAcknowledged ? (
                          ...
                          <button 
                            ...
                            className="bg-snang-teal-600 text-white px-3 py-1 rounded-lg text-xs font-medium hover:bg-snang-teal-700"
                          >
                            Sahkan
                          </button>
                        )}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {!slot.available ? (
                        ...
                        <CheckCircle className="w-5 h-5 text-snang-teal-600" />
                      ) : null}
                    </div>
                  </button>
                ))}
        ...
        return (
          <div>
            <div className="text-center mb-6">
              <div className="w-16 h-16 bg-snang-teal-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <CheckCircle className="w-8 h-8 text-snang-teal-600" />
              </div>
            ...
            <div className="bg-slate-800 rounded-xl p-4 mb-5">
              <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
                <Calendar className="w-4 h-4 text-snang-teal-500" /> Jangkaan Timeline
              </h3>
              <div className="space-y-4">
                  ...
                  <div key={idx} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full ${
                        item.status === 'done' ? 'bg-snang-teal-600' :
                        item.status === 'current' ? 'bg-snang-teal-400 animate-pulse' :
                        'bg-slate-600'
                      }`} />
                      {idx < timeline.length - 1 && (
                        <div className={`w-0.5 flex-1 mt-1 ${
                          item.status === 'done' ? 'bg-snang-teal-600' : 'bg-slate-600'
                        }`} />
                      )}
                    </div>
                    <div className="flex-1 pb-4">
                      <div className="flex justify-between items-start">
                        <p className={`font-medium text-sm ${
                          item.status === 'done' ? 'text-snang-teal-500' :
                          item.status === 'current' ? 'text-white' :
                          'text-slate-400'
                        }`}>{item.phase}</p>
                        <span className={`text-xs ${
                          item.status === 'done' ? 'text-snang-teal-500' :
                          item.status === 'current' ? 'text-snang-teal-300' :
                          'text-slate-500'
                        }`}>{item.date}</span>
                      </div>
            ...
            {/* Estimate Box */}
            <div className="bg-gradient-to-r from-snang-teal-600 to-snang-teal-700 rounded-xl p-4 mb-5 text-white">
              <p className="text-sm font-medium text-snang-teal-100">Jangkaan Siap</p>
              <p className="text-2xl font-bold">April 2026</p>
              <p className="text-xs text-snang-teal-200 mt-1">
        ...
        <div className="bg-gradient-to-r from-snang-teal-700 to-snang-teal-900 px-5 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-snang-teal-500 text-xs font-mono">LPPSA</span>
            </div>
            ...
            {steps.map((s, i) => (
              <div 
                ...
                className={`h-1 flex-1 rounded-full transition-all ${
                  i <= step ? 'bg-snang-teal-600' : 'bg-slate-700'
                }`}
              />
            ))}
          </div>
